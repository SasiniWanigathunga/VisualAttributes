...
#     return f"""In a game of object detection from images, you are given an image patch of an object. The patch taken from an image after a random crop. It is required to identify the object class correctly. Your task is to generate visual features that enable you to identify the object only from the patch.

# Provide 5 attributes list per class as the values of the dictionary
    
# Q: What are useful visual attributes for distinguishing a lemur from {' , '.join(classes)} in a photo? Those attributes should help a machine learning model to identify a lemur from a patch of an image.
...
# Q: What are useful visual attributes for distinguishing a television from {' , '.join(classes)} in a photo? Those attributes should help a machine learning model to identify a television from a patch of an image.
...
# Q: What are useful visual attributes for distinguishing a {category_name} from{' , '.join(classes)} in a photo? Those attributes should help a machine learning model to identify a {category_name} from a patch of an image.
...
#     print(classes)
#     # you can replace the examples with whatever you want; these were random and worked, could be improved
#     return f"""Provide python list of visual attributes to distinguish a lemur from a patch of an image.
# Attributes can be shape, orientation, and primary physical characteristics, material, texture, or typical colors. Use simple language.
# - ["large round eyes","long bushy tail","gray and white fur","curved ears","small face"]

# Provide python list of visual attributes to distinguish a {category_name} from a patch of an image.
# Attributes can be shape, orientation, and primary physical characteristics, material, texture, or typical colors. Use simple language.
# - 
# """

def generate_prompt(class_list: list) -> str:
    # classes = class_list.copy()
    # classes.remove(f'{category_name}')
    # print(classes)
    # you can replace the examples with whatever you want; these were random and worked, could be improved
    return f"""Give 5 different visual attributes to distinguish the following classes from each other in a segmentation task. Attributes for one class must differ from other class.
...
# obtain_descriptors_and_save("jsons/dark_zurich_sem_seg_val.json", ['unlabeled', 'road', 'sidewalk', 'building', 'wall', 'fence', 'pole', 'traffic light', 'traffic sign', 'vegetation', 'terrain', 'sky', 'person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle', 'bicycle'])
...
# obtain_descriptors_and_save("jsons/pst900_sem_seg_test.json", ["background", "fire extinguisher", "backpack", "drill", "human"])
# obtain_descriptors_and_save("jsons/dark_zurich_sem_seg_val.json", ['unlabled', 'road', 'sidewalk', 'building', 'wall', 'fence', 'pole', 'traffic light', 'traffic sign', 'vegetation', 'terrain', 'sky', 'person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle', 'bicycle'])
...
obtain_descriptors_and_save("jsons/foodseg103_sem_seg_test1.json", ['background', 'candy', 'egg tart', 'french fries', 'chocolate', 'biscuit', 'popcorn', 'pudding', 'ice cream', 'cheese butter', 'cake', 'wine', 'milkshake', 'coffee', 'juice', 'milk', 'tea', 'almond', 'red beans', 'cashew'])
# obtain_descriptors_and_save("jsons/foodseg103_sem_seg_test2.json", ['dried cranberries', 'soy', 'walnut', 'peanut', 'egg', 'apple', 'date', 'apricot', 'avocado', 'banana', 'strawberry', 'cherry', 'blueberry', 'raspberry', 'mango', 'olives', 'peach', 'lemon', 'pear', 'fig'])
# obtain_descriptors_and_save("jsons/foodseg103_sem_seg_test3.json", ['pineapple', 'grape', 'kiwi', 'melon', 'orange', 'watermelon', 'steak', 'pork', 'chicken duck', 'sausage', 'fried meat', 'lamb', 'sauce', 'crab', 'fish', 'shellfish', 'shrimp', 'soup', 'bread', 'corn'])
# obtain_descriptors_and_save("jsons/foodseg103_sem_seg_test4.json", ['hamburg', 'pizza', 'hanamaki baozi', 'wonton dumplings', 'pasta', 'noodles', 'rice', 'pie', 'tofu', 'eggplant', 'potato', 'garlic', 'cauliflower', 'tomato', 'kelp', 'seaweed', 'spring onion', 'rape', 'ginger', 'okra'])
# obtain_descriptors_and_save("jsons/foodseg103_sem_seg_test5.json", ['lettuce', 'pumpkin', 'cucumber', 'white radish', 'carrot', 'asparagus', 'bamboo shoots', 'broccoli', 'celery stick', 'cilantro mint', 'snow peas', 'cabbage', 'bean sprouts', 'onion', 'pepper', 'green beans', 'French beans', 'king oyster mushroom', 'shiitake', 'enoki mushroom', 'oyster mushroom', 'white button mushroom', 'salad', 'other ingredients'])
